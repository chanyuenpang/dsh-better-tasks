...
test('Goal and non-empty Todo are expandable only when their projections are ready', async () => {
  ...
  assert.match(source, /if \(goalAvailable\) toggleProjection\(task\.id, 'goal', preferences\.defaultGoalExpanded\)/)
  assert.match(source, /if \(todoAvailable\) toggleProjection\(task\.id, 'todo', preferences\.defaultTodoExpanded\)/)
  assert.match(source, /useStoredExpansion\('dsh-better-tasks\.projection-expansion\.v2', 'dsh-better-tasks\.projection-expansion\.v1'\)/)
  ...
  assert.match(source, /\.dbt-projection-detail\{[^}]*font-size:var\(--dbt-projection-font-size,13px\);line-height:1\.5/)
  assert.match(source, /style: \{ '--dbt-projection-font-size': `\$\{preferences\.detailFontSize\}px` \}/)
  assert.match(source, /toggleProjection\(task\.id, 'todo', preferences\.defaultTodoExpanded\)/)
  assert.match(source, /toggleProjection\(task\.id, 'goal', preferences\.defaultGoalExpanded\)/)
...
test('Better Tasks owns an optional native settings section with independent controls', async () => {
  const source = await readFile(sourceUrl, 'utf8')
  assert.match(source, /ctx\.inject\(\['settingsScope'\], \(scope\) => \{/)
  assert.match(source, /scope\.settingsScope\.bind\(\{ namespace: TASK_SETTINGS_NAMESPACE \}\)/)
  assert.match(source, /ctx\.slots\.inject\('settings\.section', \(\) => ctx\.slots\.register\(\{/)
  assert.match(source, /id: 'better-tasks',[\s\S]*?order: 45/)
  assert.match(source, /\[11, 12, 13, 14, 15, 16\]\.map/)
  assert.match(source, /switchRow\('defaultTodoExpanded'/)
  assert.match(source, /switchRow\('defaultGoalExpanded'/)
  assert.match(source, /switchRow\('defaultFinalExpanded'/)
  assert.match(source, /\['auto', 'single', 'double'\]\.map/)
  assert.match(source, /setTaskPreference: \(field, value\) => taskPreferences\.set\(field, value\)/)
  assert.match(source, /export const inject = \['slots', 'layout', 'locale'\]/)
  assert.doesNotMatch(source, /export const inject = \[[^\]]*settingsScope/)
})

test('idle Final disclosure lazily reads a minimal no-store Host projection', async () => {
  const source = await readFile(sourceUrl, 'utf8')
  assert.match(source, /task\.session\.status === 'idle' && isProjectionExpanded\(expanded, `\$\{task\.id\}:final`, preferences\.defaultFinalExpanded\)/)
