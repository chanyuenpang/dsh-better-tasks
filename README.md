...
- The Tasks view renders that exact queue order. Cards are whole-card drag surfaces and selection targets without a grab cursor, with a hover-only top-right unpin action, native `relativeTime`, the unchanged native running `StateDot`, persistent read-only Goal/Todo disclosures, and `block > running > idle` card backgrounds. Empty or completed Goal projections and unavailable Todo projections are disabled. Entering a new idle cycle automatically expands a lazy durable Final preview; its bounded safe payload and the cycle marker are cached in browser storage, so refreshes do not refetch unchanged content and a manual collapse survives tab switches/remounts until a later idle cycle.
