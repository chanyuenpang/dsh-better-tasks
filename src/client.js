...
.dbt-projection-detail{display:grid;grid-template-columns:18px minmax(0,1fr);gap:6px;align-items:start;border-radius:8px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-2) 86%,transparent);padding:7px;font-size:11px;line-height:1.35}
...
export function parseFinalMessageCache(raw) {
...
export function serializeFinalMessageCache(states) {
    ...
    tasks.length === 0 ? element('div', { className: 'dbt-task-empty' }, t('tasks.empty')) :
      element('div', {
        ...
        if (finalExpanded) {
          ...
          }, element(IconSparkle16, { size: 14 }), element('div', { className: 'dbt-projection-copy' }, finalContent)))
        }
...
export function BetterSidebarRoot({
  collapsed,
  width,
  startSession,
  toggleSidebar,
